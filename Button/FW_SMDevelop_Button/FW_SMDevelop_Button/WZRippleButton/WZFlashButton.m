...
/**
 WZFlashButton是一款能够自定义的具有闪烁效果或者涟漪效果的按钮。作者的灵感来源于RippleEffect。按钮支持点击回调的方法，使用起来很简单。
 */
...
{
    [self setText:text withTextColor:nil withTextSize:-1];
}

- (void)setTextColor:(UIColor *)textColor
{
    [self setText:nil withTextColor:textColor withTextSize:-1];
}


- (void)setText:(NSString *)text withTextColor:(UIColor *)textColor withTextSize:(int)size
{
    if (textColor) {
    ...
    if(size > 0)
    {
        UIFont *myFont = [UIFont fontWithName:self.textLabel.font.fontName size:size];
