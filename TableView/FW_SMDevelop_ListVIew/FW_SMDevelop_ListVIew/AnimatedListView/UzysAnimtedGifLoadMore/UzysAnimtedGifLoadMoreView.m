...
@property (nonatomic, strong) UILabel* _tipText;
@property (nonatomic, strong) UIView* _imageTextView;
...
#define imageTextViewViewHeight 80
#define imageTextViewViewWidth 200
#define tipTextLeftPadding 80
#pragma mark - private Method
- (void)_commonInit
{
    ProgressTipText = @"松开查看更多";
    LoadingTipText = @"正在加载中...";
    TipTextWidth = 120;
    CGRect r = [UIScreen mainScreen].applicationFrame;
    int width = r.size.width;
    self._imageTextView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, imageTextViewViewWidth, imageTextViewViewHeight)];
    self._imageTextView.center = CGPointMake(width / 2, imageTextViewViewHeight / 2);
    self._tipText = [[UILabel alloc] initWithFrame:CGRectMake(tipTextLeftPadding, 0, TipTextWidth, imageTextViewViewHeight)];
    ...
    [self._imageTextView addSubview:self._imageViewProgress];
    [self._imageTextView addSubview:self._tipText];
    [self addSubview:self._imageTextView];
    ...
    if (self._pImgArrLoading == nil) {
        ...
        [self._imageTextView addSubview:self._activityIndicatorView];
        [self._imageTextView addSubview:self._tipText];
        [self addSubview:self._imageTextView];
    }
    else {
        ...
        [self._imageTextView addSubview:self._imageViewLoading];
        [self._imageTextView addSubview:self._tipText];
        [self addSubview:self._imageTextView];
    }
...
- (void)_setFrameSizeByProgressImage
{
    ...
    if (image1)
        self.frame = CGRectMake(0 /*(self.scrollView.bounds.size.width - image1.size.width) / 2*/, self.frame.origin.y, /*image1.size.width*/ self._imageTextView.frame.size.width, /*image1.size.height*/ self._imageTextView.frame.size.height);
}
- (void)_setFrameSizeByLoadingImage
{
...
- (void)_scrollViewDidScroll:(CGPoint)contentOffset
{
    ...
    case UZYSGIFLoadMoreStateTriggering: //progress (2)
    {
        ...
        self._tipText.text = ProgressTipText;
    } break;
    ...
    case UZYSGIFLoadMoreStateLoading: //wait until stopIndicatorAnimation (4)
        self._tipText.text = LoadingTipText;
        break;
...
- (void)setLoadMoreProgressTipText:(NSString*)text
{
    ProgressTipText = text;
}
- (void)setLoadMoreLoadingTipText:(NSString*)text
{
    LoadingTipText = text;
}
- (void)setTipTextWidth:(int)width
{
    TipTextWidth = width;
}
@end
