...
- (void)setIndivatorImage:(NSString*)imageName
{
    _stateIndicatorView.image = [UIImage imageNamed:imageName];
}

- (void)didMoveToSuperview
{
    self.bounds = CGRectMake(0, 0, self.scrollView.frame.size.width,
        SDRefreshViewDefaultHeight);
}
...
- (void)layoutSubviews
{
    ...
    _timeIndicator.center = CGPointMake(
        self.sd_width * 0.5,
        self.sd_height - _timeIndicator.sd_height * 0.5 - SDTimeIndicatorMargin);
}
...
- (void)addToScrollView:(UIScrollView*)scrollView
{
    ...
    [_scrollView addObserver:self
                  forKeyPath:SDRefreshViewObservingkeyPath
                     options:NSKeyValueObservingOptionNew
                     context:nil];

    // 默认是在NavigationController控制下，否则可以调用addToScrollView:isEffectedByNavigationController:(设值为NO)
    // 即可
    _isEffectedByNavigationController = YES;
}

- (void)addToScrollView:(UIScrollView*)scrollView
    isEffectedByNavigationController:(BOOL)effectedByNavigationController
{
...
{
    return UIEdgeInsetsMake(_originalEdgeInsets.top + edgeInsets.top,
        _originalEdgeInsets.left + edgeInsets.left,
        _originalEdgeInsets.bottom + edgeInsets.bottom,
        _originalEdgeInsets.right + edgeInsets.right);
}
...
- (void)setRefreshState:(SDRefreshViewState)refreshState
{
    ...
    case SDRefreshViewStateRefreshing: {
        ...
        _scrollView.contentInset =
            [self syntheticalEdgeInsetsWithEdgeInsets:self.scrollViewEdgeInsets];
        ...
        if (self.beginRefreshingOperation) {
            ...
            if ([self.beginRefreshingTarget
                    respondsToSelector:self.beginRefreshingAction]) {
// 屏蔽performSelector-leak警告
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                [self.beginRefreshingTarget
                    performSelector:self.beginRefreshingAction];
            }
        }
    } break;
    ...
    case SDRefreshViewStateWillRefresh: {
        ...
        case Refresh:
            ...
            _timeIndicator.text =
                [NSString stringWithFormat:@"上一次更新时间：%@",
                          [self lastRefreshingTimeString]];
            break;
        case LoadMore:
            _textIndicator.text = SDRefreshViewWillLoadMoreStateText;
            _timeIndicator.text =
                [NSString stringWithFormat:@"上一次加载时间：%@",
                          [self lastRefreshingTimeString]];
        default:
            break;
        }
        ...
        [UIView
            animateWithDuration:0.5
                     ...
                     _stateIndicatorView
                         ...
                         self.stateIndicatorViewWillRefreshStateTransformAngle);
                     }];
    } break;

    case SDRefreshViewStateNormal:
        _textIndicator.text = self.textForNormalState;
        _stateIndicatorView.transform = CGAffineTransformMakeRotation(
            self.stateIndicatorViewNormalTransformAngle);
