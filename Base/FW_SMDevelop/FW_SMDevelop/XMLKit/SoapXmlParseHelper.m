...
+ (NSString*)SoapMessageResultXml:(id)data ServiceMethodName:(NSString*)methodName
{
    ...
    while ([result count] > 0) {
        ...
        if ([nodeName isEqualToString:searchStr]) {
            NSLog(@"%@", [result objectAtIndex:0]);
            GDataXMLElement* temp = [result objectAtIndex:0];
            MsgResult = [temp stringValue];
            break;
        }
...
+ (void)parseTest:(NSData*)data ServiceMethodName:(NSString*)methodName
{
    //使用NSData对象初始化
    ...
    //NSLog(@"%@",[[rootElement children] objectAtIndex:0]);

    //获取根节点下的节点（User）
    NSArray* result = [rootElement children];
    NSArray* rightresult = nil;
    NSString* searchStr = [NSString stringWithFormat:@"%@Result", methodName];
    while ([result count] > 0) {
        NSString* nodeName = [[result objectAtIndex:0] name];
        if ([nodeName isEqualToString:searchStr]) {
            //NSLog(@"%@", [result objectAtIndex:0]);
            GDataXMLElement* temp = [result objectAtIndex:0];
            rightresult = [temp children];
            break;
        }
        result = [[result objectAtIndex:0] children];
    }
    for (GDataXMLElement* user in rightresult) {
        //User节点的id属性
        GDataXMLElement* nameElement = [[user elementsForName:@"TagName"] objectAtIndex:0];
        NSString* name = [nameElement stringValue];
        NSLog(@"TagName is:%@", name);

        //获取name节点的值
        GDataXMLElement* IDElement = [[user elementsForName:@"TagID"] objectAtIndex:0];
        NSString* ID = [IDElement stringValue];
        NSLog(@"TagID is:%@", ID);

        //获取age节点的值
        GDataXMLElement* CounterElement = [[user elementsForName:@"TagCounter"] objectAtIndex:0];
        NSString* Counter = [CounterElement stringValue];
